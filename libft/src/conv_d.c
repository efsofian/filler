...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/09 16:47:56 by seliasbe          #+#    #+#             */
/*   Updated: 2017/05/27 02:28:48 by seliasbe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static char	*flags_norme(long long d, t_printf *env, char *s)
{
	if (env->plus && d >= 0)
...
static char	*flags(long long d, t_printf *env)
{
	char	*s;
	int		i;
...
static char	*zero_norme(t_printf *env, long long d, char *s)
{
	if (env->plus && d >= 0)
		s = ft_freejoin(ft_strdup("+"), s);
	if (env->space && d >= 0 && !env->plus)
...
static char	*zero(long long d, t_printf *env)
{
	char	*s;
	int		i;
